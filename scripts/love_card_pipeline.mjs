...
async function genText(t, title, chars) {
    ...
    const SEGS = lg === "en" ? [
        ...
        // 「末行写告白轮」旧版把术语写成中文:模型会把这个词原样填进表格格子里,
        // 「轮」是简体专有字 → 整段判脏(实测 kr-r1-04-choir 连卡 4 轮,病灶就是这一格)。
        // 凡是要求里点名要在正文出现的术语,一律连目标语写法一起给,别让模型自己翻译。
        { n: 380, heads: "## ラウンド表\n| 回 | シーン | 登場人物 |\n| --- | --- | --- |", reqs: "13-16 行表体,最后一格写告白那回——格子内容必须全用日语(如:告白の回)。" }
    ] : [
        ...
        { n: 380, heads: "## 라운드 표\n| 회차 | 장면 | 등장인물 |\n| --- | --- | --- |", reqs: "13-16 行表体,最后一格写告白那轮——格子内容必须全用韩语(如:고백 회차)。" }
    ];
