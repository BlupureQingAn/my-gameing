...
async function repairDirtyLines(t, raw) {
    ...
    // 只对 ko 说这句:模型不认为韩语里的汉字(한자)算"中文",原提示说"混进了中文"它对不上号,
    // 于是反复改不掉「告白」这种汉字词。给它到谚文的对应关系,它一次就改对。
    const hanNote = lg === "ko" ? `\n注意:韩语正文里不用汉字(한자),像「告白」这类汉字词必须改写成谚文(고백),整行除了标点和数字不能有汉字。` : "";
    const d = await askText(sp.sysText,
        `下面这段${sp.label}文本里有几行混进了中文或其他语种的字` + (badChars ? `,问题字就是这些:「${badChars}」` : "") + `。` + hanNote + `\n` +
...
async function genText(t, title, chars) {
    ...
    const SEGS = lg === "en" ? [
        ...
        // 同 ko:要求里凡是点名要写的术语,只给日语写法,不留中文词给模型抄(旧「末行写告白轮」的
        // 「轮」是简体专有字,抄进格子直接判脏)。
        // 行数/字数对齐模型的实际能力:旧规格「13-16 行表体」+ n=380 从未被满足过
        // (同批 7 张卡实得 4-10 行/127-1009 字),结果只是让长度闸门每轮白跑 3 次重试。
        // 表尾落在告白回这个语义目标才是重点,行数是装饰。
        { n: 200, heads: "## ラウンド表\n| 回 | シーン | 登場人物 |\n| --- | --- | --- |", reqs: "6-10 行表体,最后一格写「告白の回」。整张表格每一格都必须全用日语。" }
    ] : [
        ...
        // 这一格是全场最高频的脏点:模型特别爱把「告白」写成汉字。「告白那轮」这种写法自己就是病灶
        // ——要求里出现的中文词会被原样抄进格子(实测连写三次 |告白|、|告白轮|),所以要求里连
        // 「告白」两个字都不许出现,只给谚文写法。
        { n: 200, heads: "## 라운드 표\n| 회차 | 장면 | 등장인물 |\n| --- | --- | --- |", reqs: "6-10 行表体,最后一格写「고백 회차」。整张表格每一格都必须全用韩语谚文,一个汉字都不许出现。" }
    ];
